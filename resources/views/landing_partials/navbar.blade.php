    ...
    <div class="container mx-auto flex justify-between items-center py-4">
        <a href="/" class="text-2xl font-bold flex items-center lg:ml-2.5">
            ...
            {{-- <form class="sm:pr-3 mb-4 sm:mb-0" action="{{route('landingpage.event.search')}}" method="GET">
                ...
                <input type="text" name="search" id="search" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="Search for {{__('welcome.events')}}">
                </div>
            </form> --}}
        <nav>
            <ul class="flex space-x-6">
                @auth
                <li><a href="{{route('myTicket')}}" class="text-gray-700 hover:text-blue-500 text-xl">{{__('welcome.my_tickets')}}</a></li>
                @endauth
                <li><a href="{{url('/')}}" class="text-gray-700 hover:text-blue-500 text-xl">{{__('welcome.events')}}</a></li>
